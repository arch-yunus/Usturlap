...
async def get_chart(datetime_str: str = Query(..., alias="datetime"), lat: float = Query(...), lon: float = Query(...), system: str = Query("placidus")):
    try:
...
async def synastry(req: SynastryRequest):
    try:
        c1 = engine.calculate_chart(req.person_1.datetime, req.person_1.lat, req.person_1.lon, HOUSE_SYSTEMS.get(req.person_1.house_system.lower(), "P"))
        c2 = engine.calculate_chart(req.person_2.datetime, req.person_2.lat, req.person_2.lon, HOUSE_SYSTEMS.get(req.person_2.house_system.lower(), "P"))
...
async def transits(req: TransitRequest):
    try:
        natal = engine.calculate_chart(req.natal.datetime, req.natal.lat, req.natal.lon, HOUSE_SYSTEMS.get(req.natal.house_system.lower(), "P"))
